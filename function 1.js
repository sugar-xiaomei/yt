...
function insertAfter(obj, after) {
	...
	if (next) {
		...
		parent.appendChild(obj)
             ...
             //添加事件
function   addEvent(obj,event,shijian){
	if(obj.addEventListener){
        obj.addEventListener(event,shijian,false);
	}else if(obj.attachEvent){
		event="on"+event;
		obj.attachEvent(event,shijan);
	}
}
             //删除事件
function   deleteEvent(obj,event,shijian){
	if(obj.detachEvent){
		event="on"+event;
		obj.detachEvent(event,shijian);
	}else if(obj.removeEventListener){
		obj.removeEventListener(event,shijian,false);
	}
}
      //滚轮滚动事件
